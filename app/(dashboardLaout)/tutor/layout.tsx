  ...
  if (loading) {
    return (
      <div className="h-screen flex items-center justify-center bg-gray-50">
        <LoadingSpinner size={48} text="Tutor dashboard loading..." />
      </div>
    );
  }
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          ...
          <nav className="space-y-2">
            {navItem("/tutor/dashboard", "Dashboard", LayoutDashboard)}
            {navItem("/tutor/dashboard/sessions", "My Sessions", BookOpen)}
            {navItem("/tutor/dashboard/availability", "Availability", CalendarClock)}
            
            {navItem("/tutor/dashboard/profile", "Profile", User)}
            
          </nav>

          {/* Footer */}
          <div className="absolute bottom-4 text-xs text-gray-400">
            Tutor Panel • SkillBridge
          </div>
        </aside>
